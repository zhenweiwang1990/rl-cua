#!/bin/bash
# AReaL 训练脚本（单 GPU，独立运行，连接到已启动的 vLLM）
...
echo "GPU: Single GPU (shared with vLLM)"
echo "=========================================="
echo ""
...
while ! curl -s -f "${VLLM_URL}/health" > /dev/null 2>&1; do
    if [ $WAITED -ge $MAX_WAIT ]; then
        ...
        echo "  ./scripts/start_vllm_single_gpu.sh"
        echo ""
        echo "Or check if vLLM is running:"
        echo "  docker ps | grep vllm-cua-areal-single"
        exit 1
    fi
...
docker compose -f docker-compose.trainer.single-gpu.yml build
...
# 确保连接到 vLLM 网络（如果网络不存在，先创建）
NETWORK_NAME="vllm-cua-areal-single_vllm-network"
if ! docker network ls | grep -q "$NETWORK_NAME"; then
    echo "Creating vLLM network..."
    docker network create "$NETWORK_NAME" 2>/dev/null || true
fi

# 启动训练服务（连接到 vLLM 网络）
docker compose -f docker-compose.trainer.single-gpu.yml up trainer
