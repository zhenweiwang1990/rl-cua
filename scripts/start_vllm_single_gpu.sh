#!/bin/bash
# 启动 vLLM 服务（单 GPU，持续运行，always restart）
...
# ============ 检查 vLLM 是否已在运行 ============
if docker ps --format '{{.Names}}' | grep -q "^vllm-cua-areal-single$"; then
    echo "vLLM 服务已在运行中"
    echo "容器状态:"
    docker ps --filter "name=vllm-cua-areal-single" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
    echo ""
...
# ============ 显示配置 ============
echo "=========================================="
echo "vLLM Service Configuration (Single GPU)"
echo "=========================================="
...
# ============ 启动 vLLM 服务 ============
echo "Starting vLLM service (single GPU, with restart: always)..."
...
echo "To stop: ./scripts/stop_vllm_single_gpu.sh"
